############################################################
## Early vs Late CCW simulation
## Public analysis code using unstabilized inverse probability
...
# Clone-censor-weight implementation with unstabilized IPCW
...
estimate_ipcw_weights_unstabilized <- function(clone_dat,
                                                t_max = 11,
                                                cap_w = 20,
                                                eps = 1e-6) {
  ...
  if (nrow(pp) == 0) {
    stop("No person-period rows were available for IPCW estimation.")
  }
  ...
  pp <- pp |>
    mutate(
      pcens_hat = pmin(pmax(pcens_hat, eps), 1 - eps)
    )
  ...
  clone_dat |>
    ...
    mutate(
      G_hat = ifelse(is.na(G_hat), 1, G_hat),
      w_ipcw = ifelse(G_hat > 0, 1 / G_hat, NA_real_),
      w_ipcw = pmin(w_ipcw, cap_w)
    )
}
...
analyze_one_window <- function(dat,
  ...
  clone_w <- estimate_ipcw_weights_unstabilized(clone_dat, t_max = t_max, cap_w = 20)
...
p_rr <- ggplot(
  ...
  labs(
    ...
    shape = "Simulation\nsetting",
    linetype = "Simulation\nsetting",
    title = "Primary analysis with unstabilized IPCW: Early-Late risk ratios across window families"
  ) +
...
p_ess <- ggplot(
  ...
  labs(
    ...
    shape = "Simulation\nsetting",
    linetype = "Simulation\nsetting",
    title = "Primary analysis with unstabilized IPCW: effective sample size across window families"
  ) +
  scale_shape_manual(values = shape_vals) +
  scale_linetype_manual(values = linetype_vals) +
  theme_bw() +
...
############################################################
## Sim3 Monte Carlo bias analysis with unstabilized IPCW: Sliding windows only
############################################################
...
############################################################
## Early vs Late CCW simulation
## Public analysis code using stabilized inverse probability
## of censoring weights (IPCW)
############################################################
